...
class forms_BreedingCage extends Zend_Dojo_Form  {
	...
	public function __construct($options = null) {
		...
		$breeding_type->setLabel ( 'Breeding Type' )
			->setStoreId('breeding_type_store')
			->setStoreType('dojo.data.ItemFileReadStore')
			->setStoreParams(array('url' => $baseUrl.'/breeding-cage/suggest/field/breeding_type/format/json'))
		...
		$mating_type = new Zend_Dojo_Form_Element_FilteringSelect( 'mating_type' );
		$mating_type->setLabel ( 'Mating Type' )
			->setStoreId('mating_type_store')
			->setStoreType('dojo.data.ItemFileReadStore')
			->setStoreParams(array('url' => $baseUrl.'/breeding-cage/suggest/field/mating_type/format/json'))
			->setAutocomplete(false)
			->setDijitParam('hasDownArrow', 'true')
			->setDijitParam('searchAttr', 'item')
			->setDijitParam('searchDelay', 200)
        ...
        $this->addElements ( array ($id, $user_id, $lastmodified, $assigned_id, $active, $set_up_on,
			$breeding_type, $mating_type, $protocol_id, $submit, $delete) );

		$this->addDisplayGroup ( array ('assigned_id', 'active', 'set_up_on'), 'left' );
		$this->addDisplayGroup( array ('breeding_type', 'mating_type', 'protocol_id'), 'right' );
